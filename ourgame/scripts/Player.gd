...
@export var player_knockback_strength = 400.0
@export var knockback_duration = 0.2

#@onready var sfx_walk: AudioStreamPlayer2D = $sfx_walk
@onready var sfx_jump: AudioStreamPlayer2D = $sfx_jump
@onready var sfx_slash: AudioStreamPlayer2D = $sfx_slash
@onready var sfx_bonk: AudioStreamPlayer2D = $sfx_bonk
...
func handle_movement():
	...
	# --- Only the BASIC form can jump now ---
	if current_form == Form.BASIC:
		...
		# No jump in FIRST_TRANSFORMATION or SECOND_TRANSFORMATION
		if is_on_floor():
			is_jumping = false
...
func take_damage(amount: int, attacker_pos: Vector2 = Vector2.ZERO):
	...
	# Sibling path: parent -> "HUD"
	var hud = get_node("../HUD")  # Goes up one level, looks for "HUD"
